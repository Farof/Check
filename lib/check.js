...
(function Check_closure(exports) {
  ...
  ValidatorBuilder = (function () {
    var Builder = function Check_ValidatorBuilder() {
      this.rules = [];
    };
    Check.rules = Builder.prototype;
    return Builder;
  }()),
   ...
   *
   *  It has also access to two helpers :
   *
   ...
   * @return Check
   */
  Rule = function Check_Rule(name, test, message) {
    var rule = function Check_rule() {
      var
      params = Array.prototype.slice.call(arguments),
      
      message = ValidatorBuilder.prototype[name].errorMsg,
      ...
      throwError = function Check_rule_throwError(val) {
        var msg, e, i, ln, join;
        ...
        if (message) {
          ...
          msg = msg.replace('{val}', val, 'g')
                    ...
                    .replace('{join:val}', function rule_error_val_replace_join() {
                      return val.join(', ');
                    }, 'g');

          join = function rule_error_param_replace_join() {
        ...
        e.val = val;
        e.params = params;
        e.rule = ValidatorBuilder.prototype[name];
        throw e;
      },

      evaluate = function Check_rule_evaluate(val, callback) {
        var
        resultHandled = false,

        handleResult = function evaluate_handleResult(err, result) {
          if (resultHandled) {
            return;
          }
          resultHandled = true;
        ...
        isValid = test.call(ctx, val, handleResult);
      ...
      this.rules.push(evaluate);

      return this;
    };
    rule.name = name;
    rule.test = test;
    rule.errorMsg = message || 'Default rule message';

    return rule;
  };


  Check.addRule = function Check_addRule(name, test, message) {
    ValidatorBuilder.prototype[name] = new Rule(name, test, message);
    return Check;
  };
   ...
   * @return {function}
   */
  Check.build = function Check_build(rules) {
    var
    ...
    return function Check_validator(val, callback) {
      var
      ...
      next = function validator_handle_next(err, result) {
