...
(function Check_closure(exports) {
      ...
      throwError = function (value) {
        ...
        if ((!negation && self.msg) || (negation && self.negMsg)) {
          ...
          errorMsg = errorMsg.replace('{val}', value)
                              ...
                              .replace('{join:val}', Array.isArray(value) ? value.join(', ') : value)
  ...
  Check.addRule('length', function (val) {
    return this.assert.equal(val.length, this.params[0]);
  }, 'Expected [{join:val}] to have a length of {0}', 'Expected [{join:val}] to not have a length of {0}');
  
  Check.addRule('lengthAtLeast', function (val) {
    return this.assert.atLeast(val.length, this.params[0]);
  }, 'Expected [{join:val}] length to be at least {0}', 'Expected [{join:val}] be shorter than {0}');
  
  Check.addRule('lengthAtMost', function (val) {
    return this.assert.atMost(val.length, this.params[0]);
  }, 'Expected [{join:val}] length to be at most {0}', 'Expected [{join:val}] to be longer than {0}');
  
  Check.addRule('longerThan', function (val) {
    return this.assert.greaterThan(val.length, this.params[0]);
  }, 'Expected [{join:val}] to be longer than {0}', 'Expected [{join:val}] to be at most {0}');
  
  Check.addRule('shorterThan', function (val) {
    return this.assert.lessThan(val.length, this.params[0]);
  }, 'Expected [{join:val}] to be shorter than {0}', 'Expected [{join:val}] to be at least {0}');
  
  Check.addRule('lengthBetween', function (val) {
    return this.assert.atLeast(val.length, this.params[0]) && this.assert.atMost(val.length, this.params[1]);
  }, 'Expected [{join:val}] length to be between {0} and {1}', 'Expected [{join:val}] length to not be between {0} and {1}');
